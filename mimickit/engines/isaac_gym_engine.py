...
class IsaacGymEngine(engine.Engine):
            ...
            if self._has_dof():
                ...
                if dof_obj_ids.shape[0] > 0:
                    self._gym.set_dof_state_tensor_indexed(
                        ...
                        gymtorch.unwrap_tensor(dof_obj_ids),
                        len(dof_obj_ids),
                    )
    ...
    def create_obj(
        ...
        obj_id = self._gym.create_actor(
            env_ptr, asset, start_pose, name, col_group, col_filter, segmentation_id
        )
    ...
    def get_obj_dof_limits(self, env_id, obj_id):
        ...
        for i, (l, h) in enumerate(zip(low_arr, high_arr)):
            ...
            if l == 0 and h == 0:
                raise ValueError(
                    f"Env {env_id} Obj {obj_id} DoF {i}: both lower and upper limits are 0.0 — this may indicate a fixed joint or missing limits. Either upper or lower limit must be non-zero for all DoFs."
                )

            # infinite or NaN bounds
            if not np.isfinite(l) or not np.isfinite(h):
                raise ValueError(
                    f"Env {env_id} Obj {obj_id} DoF {i}: invalid bound detected (lower={l}, upper={h})."
                )
    ...
    def _create_simulator(self, sim_timestep, visualize):
        ...
        sim = self._gym.create_sim(
            compute_device_id, graphics_device_id, physics_engine, sim_params
        )
    ...
    def _pre_sim_step(self):
        ...
        if self._has_body_forces:
            self._gym.apply_rigid_body_force_tensors(
                self._sim,
                gymtorch.unwrap_tensor(self._body_forces_raw),
                None,
    ...
    def _build_sim_tensors(self):
        ...
        root_state = self._root_state_raw.view(
            [num_envs, self._objs_per_env, self._root_state_raw.shape[-1]]
        )
        ...
        body_state = self._body_state_raw.view(
            [num_envs, bodies_per_env, self._body_state_raw.shape[-1]]
        )
        ...
        self._need_reset_buf = torch.zeros(
            num_objs, device=self._device, dtype=torch.bool
        )
        self._objs_need_reset = self._need_reset_buf.view(
            (num_envs, self._objs_per_env)
        )
    ...
    def _build_obj_dof_dims(self):
        ...
        obj_dof_dims = torch.zeros(
            num_envs * objs_per_env, device=self._device, dtype=torch.int
        )
