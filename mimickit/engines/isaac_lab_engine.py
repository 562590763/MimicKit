...
class IsaacLabEngine(engine.Engine):
    ...
    def set_body_pos(self, env_id, obj_id, body_pos):
        ...
        if env_id is None:
            ...
            obj.data.body_link_pose_w[env_id, :, :2] += self._env_offsets[
                env_id
            ].unsqueeze(-2)
    ...
    def draw_lines(self, env_id, start_verts, end_verts, cols, line_width):
        ...
        self._draw_interface.draw_lines(
            start_pts.tolist(), end_pts.tolist(), cols.tolist(), line_widths
        )
        return
    ...
    def _build_lights(self):
        ...
        light_quat = torch_util.quat_from_euler_xyz(
            torch.tensor(0.7), torch.tensor(0.0), torch.tensor(0.6)
        )
        ...
        distant_light_cfg = sim_utils.DistantLightCfg(
            intensity=2000.0, color=(0.8, 0.8, 0.8)
        )
        self._distant_light = distant_light_cfg.func(
            distant_light_path + "/distant_light", distant_light_cfg
        )

        dome_light_cfg = sim_utils.DomeLightCfg(intensity=800.0, color=(0.7, 0.7, 0.7))
        self._dome_light = dome_light_cfg.func(
            LIGHT_PATH + "/dome_light", dome_light_cfg
        )
        return
    ...
    def _create_simulator(self, sim_timestep, visualize):
        ...
        sim_cfg = sim_utils.SimulationCfg(
            device=self._device, dt=sim_timestep, render_interval=self._sim_steps
        )
    ...
    def _build_actuator_cfg(self, control_mode):
        ...
        if control_mode == engine.ControlMode.none:
            actuator_cfg = IdealPDActuatorCfg(
                joint_names_expr=[".*"], stiffness=0, damping=0, effort_limit=0
            )
        elif control_mode == engine.ControlMode.pos:
            actuator_cfg = ImplicitActuatorCfg(
                joint_names_expr=[".*"], stiffness=None, damping=None, effort_limit=None
            )
        elif control_mode == engine.ControlMode.vel:
            actuator_cfg = ImplicitActuatorCfg(
                joint_names_expr=[".*"], stiffness=0, damping=None, effort_limit=None
            )
        elif control_mode == engine.ControlMode.torque:
            actuator_cfg = IdealPDActuatorCfg(
                joint_names_expr=[".*"], stiffness=0, damping=0, effort_limit=None
            )
    ...
    def _build_obj(self, obj_id):
        ...
        for env_id in range(num_envs):
            if not created[env_id]:
                ...
                for j in range(env_id + 1, num_envs):
                    if not created[j]:
                        ...
                        if other_asset_file == asset_file:
                            assert obj_cfg.is_valid_clone(
                                other_obj_cfg
                            ), "Cloning requires all objects that share the same asset file to have the same configurations."
                            clone_envs.append(j)
                ...
                if num_clones > 0:
                    self._clone_obj_prim(
                        source_env_id=env_id, obj_id=obj_id, clone_env_ids=clone_envs
                    )
                ...
                Logger.print(
                    "Building Obj:{:d} in {:d}/{:d} envs".format(
                        obj_id, num_created, num_envs
                    ),
                    end="\r",
                )
    ...
    def _build_rigid_prim(self, env_id, obj_id, obj_cfg):
        ...
        if color is not None:
            visual_material = sim_utils.PreviewSurfaceCfg(
                diffuse_color=(color[0], color[1], color[2]), metallic=0.5
            )
        else:
            visual_material = None

        rigid_props = sim_utils.RigidBodyPropertiesCfg(
            kinematic_enabled=obj_cfg.fix_root,
        ...
        init_state = RigidObjectCfg.InitialStateCfg(
            pos=obj_cfg.start_pos, rot=obj_cfg.start_rot
        )

        rigid_cfg = RigidObjectCfg(
            prim_path=prim_path, spawn=usd_cfg, collision_group=0, init_state=init_state
        )
    ...
    def _build_multi_articulated_prim(self, obj_id, obj_cfg):
        ...
        regex = OBJ_PATH_TEMPLATE.format(".*", obj_id)
        multi_obj_cfg = ArticulationCfg(
            prim_path=regex, spawn=None, actuators={"actuators": actuator_cfg}
        )
    ...
    def _disable_prim_collisions(self, prim_path):
        ...
        child_prims = sim_utils.get_all_matching_child_prims(
            prim_path, traverse_instance_prims=True
        )
        ...
        for obj_id in range(objs_per_env):
            ...
            if obj_type == engine.ObjType.articulated:
                ...
                (
                    ...
                    body_order_common2sim,
                    dof_order_sim2common,
                    dof_order_common2sim,
                ) = self._build_body_order(obj)

                body_order_sim2common = torch.tensor(
                    body_order_sim2common, device=self._device, dtype=torch.long
                )
                body_order_common2sim = torch.tensor(
                    body_order_common2sim, device=self._device, dtype=torch.long
                )
                dof_order_sim2common = torch.tensor(
                    dof_order_sim2common, device=self._device, dtype=torch.long
                )
                dof_order_common2sim = torch.tensor(
                    dof_order_common2sim, device=self._device, dtype=torch.long
                )
            else:
                body_order_sim2common = torch.zeros(
                    1, device=self._device, dtype=torch.long
                )
                body_order_common2sim = torch.zeros(
                    1, device=self._device, dtype=torch.long
                )
                dof_order_sim2common = torch.zeros(
                    1, device=self._device, dtype=torch.long
                )
                dof_order_common2sim = torch.zeros(
                    1, device=self._device, dtype=torch.long
                )
        ...
        for obj_id in range(objs_per_env):
            ...
            for prim_child in prim_children:
                ...
                if len(prim_grandchildren) > 0:
                    ...
                    has_contact_api = prim_grandchild.HasAPI(
            ...
            if contact_prim_path is not None:
                ...
                sensor_regex = OBJ_PATH_TEMPLATE.format(
                    ".*", obj_id
                ) + "/{:s}/.*".format(contact_prim_name)

                sensor_cfg = ContactSensorCfg(
                    prim_path=sensor_regex,
    ...
    def _build_body_order(self, obj):
        ...
        body_order_common2sim = [
            body_order_sim2common.index(i) for i in range(len(body_order_sim2common))
        ]
        dof_order_common2sim = [
            dof_order_sim2common.index(i) for i in range(len(dof_order_sim2common))
        ]
    ...
    def _build_sim_tensors(self):
        ...
        self._objs_need_reset = torch.zeros(
            [num_envs, num_objs], device=self._device, dtype=torch.bool
        )
        return
    ...
    def _filter_env_collisions(self):
        ...
        self._cloner.filter_collisions(
            physics_scene_path, "/World/collisions", env_prim_paths, global_paths=[]
        )
        return
