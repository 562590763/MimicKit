...
class BaseAgent(torch.nn.Module):
    ...
    def train_model(
        self, max_samples, out_model_file, int_output_dir, logger_type, log_file
    ):
        ...
        while self._sample_count < max_samples:
            ...
            output_iter = (self._iter % self._iters_per_output == 0) or (
                self._sample_count >= max_samples
            )
    ...
    def _build_action_normalizer(self):
        ...
        if isinstance(a_space, spaces.Box):
            a_mean = torch.tensor(
                0.5 * (a_space.high + a_space.low), device=self._device, dtype=a_dtype
            )
            a_std = torch.tensor(
                0.5 * (a_space.high - a_space.low), device=self._device, dtype=a_dtype
            )
            ...
            assert (
                (a_std > 0).all().item()
            ), "init_std must be > 0 for action normalizer (Box action space wrong! Check your XML file. Joints must have 'limited=true' and non-zero bounds.)"
    ...
    def _build_return_tracker(self):
        self._train_return_tracker = return_tracker.ReturnTracker(
            self.get_num_envs(), self._device
        )
        self._test_return_tracker = return_tracker.ReturnTracker(
            self.get_num_envs(), self._device
        )
        return
