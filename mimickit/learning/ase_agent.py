...
class ASEAgent(amp_agent.AMPAgent):
    ...
    def _build_latent_buf(self):
        ...
        self._latent_buf = torch.zeros(
            [num_envs, z_dim], dtype=torch.float32, device=self._device
        )
        self._latent_reset_time = torch.zeros(
            [num_envs], dtype=torch.float32, device=self._device
        )
        return
    ...
    def _decide_action(self, obs, info):
        ...
        a_info = {"a_logp": norm_a_logp, "rand_action_mask": rand_action_mask}
        return a, a_info
    ...
    def _reset_latents(self, env_ids=None):
        ...
        if len(env_ids) > 0:
            ...
            dt = (
                self._latent_time_max - self._latent_time_min
            ) * t + self._latent_time_min
    ...
    def _compute_rewards(self):
        ...
        r = (
            self._task_reward_weight * task_r
    ...
    def _calc_enc_rewards(self, tar_latents, norm_enc_obs):
        with torch.no_grad():
            ...
            enc_pred = torch_util.eval_minibatch(
                self._model.eval_enc, enc_inputs, self._enc_eval_batch_size
            )
    ...
    def _compute_actor_loss(self, batch):
        ...
        actor_loss1 = adv * torch.clamp(
            a_ratio, 1.0 - self._ppo_clip_ratio, 1.0 + self._ppo_clip_ratio
        )
    ...
    def _compute_diversity_loss(self, norm_obs, action_dist, latents):
        assert isinstance(
            action_dist, distribution_gaussian_diag.DistributionGaussianDiag
        )
