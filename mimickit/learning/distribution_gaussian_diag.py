...
class DistributionGaussianDiagBuilder(torch.nn.Module):
    ...
    def _build_params(self, in_size, out_size, init_std, init_output_scale):
        ...
        torch.nn.init.uniform_(
            self._mean_net.weight, -init_output_scale, init_output_scale
        )
        ...
        if self._std_type == StdType.FIXED:
            self._logstd_net = torch.nn.Parameter(
                torch.zeros(out_size, requires_grad=False, dtype=torch.float32),
                requires_grad=False,
            )
            torch.nn.init.constant_(self._logstd_net, logstd)
        elif self._std_type == StdType.CONSTANT:
            self._logstd_net = torch.nn.Parameter(
                torch.zeros(out_size, requires_grad=True, dtype=torch.float32),
                requires_grad=True,
            )
            torch.nn.init.constant_(self._logstd_net, logstd)
            ...
            torch.nn.init.uniform_(
                self._logstd_net.weight, -init_output_scale, init_output_scale
            )
...
class DistributionGaussianDiag:
    ...
    def kl(self, other):
        ...
        res = torch.sum(
            other.logstd
            - self._logstd
            + (torch.square(self._std) + torch.square(self._mean - other.mean))
            / (2.0 * other_var),
            dim=-1,
        )
        res += -0.5 * self._dim
