...
class KinCharModel:
    ...
    def init(
        self, body_names, parent_indices, local_translation, local_rotation, joints
    ):
        ...
        self._parent_indices = torch.tensor(
            parent_indices, device=self._device, dtype=torch.long
        )
        self._local_translation = torch.tensor(
            np.array(local_translation), device=self._device, dtype=torch.float32
        )
        self._local_rotation = torch.tensor(
            np.array(local_rotation), device=self._device, dtype=torch.float32
        )
        self._joints = joints
    ...
    def load_char_file(self, char_file):
        ...
        def _add_xml_body(xml_node, parent_index, body_index, default_joint_type):
            ...
            if body_index == 0:
                ...
                curr_joint = self._parse_joint(
                    body_name, joint_data, default_joint_type
                )
            ...
            for child_body in xml_node.findall("body"):
                body_index = _add_xml_body(
                    child_body, curr_index, body_index, default_joint_type
                )
    ...
    def compute_dof_vel(self, joint_rot0, joint_rot1, dt):
        ...
        dof_vel = torch.zeros(
            dof_shape, device=joint_rot0.device, dtype=joint_rot0.dtype
        )
    ...
    def _build_root_joint(self):
        joint = Joint(name="root", joint_type=JointType.ROOT, axis=None)
        return joint
    ...
    def _parse_hinge_joint(self, xml_joint_data):
        ...
        joint_axis = np.fromstring(
            xml_joint_data.attrib.get("axis"), dtype=float, sep=" "
        )
        joint_axis = torch.tensor(joint_axis, device=self._device, dtype=torch.float32)

        joint = Joint(name=joint_name, joint_type=JointType.HINGE, axis=joint_axis)
        return joint
    ...
    def _parse_sphere_joint(self, xml_joint_data, default_joint_type):
        ...
        if is_spherical:
            ...
            joint = Joint(name=joint_name, joint_type=JointType.SPHERICAL, axis=None)
        else:
    ...
    def _parse_fixed_joint(self, body_name):
        joint = Joint(name=body_name, joint_type=JointType.FIXED, axis=None)
        return joint
    ...
    def _build_body_xml(self, body_children, body_id):
        ...
        if len(children) > 0:
            for child_id in children:
                child_xml = self._build_body_xml(
                    body_children=body_children, body_id=child_id
                )
        ...
        is_root = body_id == 0
        if is_root:
            body_xml = root_template.format(
                body_name, joint_xml, geom_xml, children_xml
            )
        else:
            body_xml = body_template.format(
                body_name,
                pos[0],
                pos[1],
                pos[2],
    ...
    def _build_joint_xml(self, body_children, body_id):
        ...
        if is_root:
            ...
            <joint name="{:s}" type="hinge" axis="{:.4f} {:.4f} {:.4f}" range="-180 180" stiffness="100" damping="10" armature=".01"/>"""
                joint_xml = joint_template.format(
                    body_name, j_axis[0], j_axis[1], j_axis[2]
                )
    ...
    def _build_geom_xml(self, body_children, body_id):
        ...
        for c in children:
            ...
            if child_bone_len > 0:
                ...
                bone_xml = bone_template.format(
                    body_name, geom_pos[0], geom_pos[1], geom_pos[2], child_bone_radius
                )
                geom_xml += bone_xml
    ...
    def _build_actuators_xml(self):
        ...
        for j in range(1, num_joints):
            ...
            if joint_type == JointType.HINGE:
                ...
                actuator_xml += motor_template.format(
                    body_name + "_y", body_name + "_y"
                )
                actuator_xml += motor_template.format(
                    body_name + "_z", body_name + "_z"
                )
            elif joint_type == JointType.FIXED:
                actuator_xml = ""
            else:
