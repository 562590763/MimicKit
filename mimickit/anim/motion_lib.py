...
class MotionLib:
    ...
    def _load_motion_pkl(self, motion_file):
        ...
        for f in range(num_motion_files):
            ...
            root_drot = torch_util.quat_diff(
                root_rot[..., :-1, :], root_rot[..., 1:, :]
            )
        ...
        self._motion_fps = torch.tensor(
            self._motion_fps, dtype=torch.float32, device=self._device
        )
        self._motion_dt = torch.tensor(
            self._motion_dt, dtype=torch.float32, device=self._device
        )
        self._motion_num_frames = torch.tensor(
            self._motion_num_frames, dtype=torch.long, device=self._device
        )
        self._motion_lengths = torch.tensor(
            self._motion_lengths, dtype=torch.float32, device=self._device
        )
        self._motion_loop_modes = torch.tensor(
            self._motion_loop_modes, dtype=torch.int, device=self._device
        )
        ...
        self._motion_frames = torch.tensor(
            self._motion_frames, dtype=torch.float32, device=self._device
        )
        ...
        self._motion_ids = torch.arange(
            num_motions, dtype=torch.long, device=self._device
        )
