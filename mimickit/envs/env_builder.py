...
def build_env(env_file, num_envs, device, visualize):
    ...
    if env_name == "char":
        ...
        env = char_env.CharEnv(
            config=env_config, num_envs=num_envs, device=device, visualize=visualize
        )
    elif env_name == "deepmimic":
        import envs.deepmimic_env as deepmimic_env

        env = deepmimic_env.DeepMimicEnv(
            config=env_config, num_envs=num_envs, device=device, visualize=visualize
        )
    elif env_name == "amp":
