...
class AMPEnv(deepmimic_env.DeepMimicEnv):
    ...
    def _fetch_disc_demo_data(self, motion_ids, motion_times0):
        ...
        time_steps = -self._engine.get_timestep() * torch.arange(
            0, self._num_disc_obs_steps, device=self._device
        )
    ...
    def _build_disc_obs_buffers(self):
        ...
        self._disc_hist_joint_rot = circular_buffer.CircularBuffer(
            batch_size=num_envs,
            buffer_len=n,
        ...
        self._disc_hist_body_pos = circular_buffer.CircularBuffer(
            batch_size=num_envs,
            buffer_len=n,
        ...
        self._disc_obs_buf = torch.zeros(
            [num_envs] + list(disc_obs_space.shape),
            device=self._device,
            dtype=disc_obs_dtype,
        )
    ...
    def _update_done(self):
        ...
        motion_len_term = torch.full(
            motion_len.shape, False, device=motion_len.device, dtype=torch.bool
        )
        ...
        self._done_buf[:] = deepmimic_env.compute_done(
...
def compute_disc_vel_obs(ref_root_rot, root_vel, root_ang_vel, dof_vel, global_obs):
    ...
    if not global_obs:
        ...
        root_vel_obs_flat = torch.reshape(
            root_vel, [root_vel.shape[0] * root_vel.shape[1], root_vel.shape[2]]
        )
        root_vel_obs_flat = torch_util.quat_rotate(
            heading_inv_rot_flat, root_vel_obs_flat
        )
        root_vel_obs = torch.reshape(root_vel_obs_flat, root_vel.shape)

        root_ang_vel_obs_flat = torch.reshape(
            root_ang_vel,
            [root_ang_vel.shape[0] * root_ang_vel.shape[1], root_ang_vel.shape[2]],
        )
        root_ang_vel_obs_flat = torch_util.quat_rotate(
            heading_inv_rot_flat, root_ang_vel_obs_flat
        )
...
def compute_disc_obs(
    ...
    pos_obs = deepmimic_env.compute_tar_obs(
        ref_root_pos=ref_root_pos,
        ref_root_rot=ref_root_rot,
        root_pos=root_pos,
        root_rot=root_rot,
