...
class DeepMimicEnv(char_env.CharEnv):
    def __init__(self, config, num_envs, device, visualize):
        ...
        self._ref_char_offset = torch.tensor(
            env_config["ref_char_offset"], device=device, dtype=torch.float
        )
    ...
    def _build_sim_tensors(self, config):
        ...
        self._motion_time_offsets = torch.zeros(
            num_envs, device=self._device, dtype=torch.float32
        )
    ...
    def _load_motions(self, motion_file):
        self._motion_lib = motion_lib.MotionLib(
    ...
    def _parse_joint_err_weights(self, joint_err_w):
        ...
        if joint_err_w is None:
            self._joint_err_w = torch.ones(
                num_joints - 1, device=self._device, dtype=torch.float32
            )
        else:
            self._joint_err_w = torch.tensor(
                joint_err_w, device=self._device, dtype=torch.float32
            )
        ...
        self._dof_err_w = torch.zeros(
            dof_size, device=self._device, dtype=torch.float32
        )
    ...
    def _reset_ref_char(self, env_ids):
        ...
        self._engine.set_root_ang_vel(
            env_ids, ref_char_id, self._ref_root_ang_vel[env_ids]
        )
    ...
    def _build_data_buffers(self):
        ...
        if self._log_tracking_error:
            num_track_errors = 7
            self._error_tracker = stats_tracker.StatsTracker(
                num_track_errors, device=self._device
            )
    ...
    def _compute_obs(self, env_ids=None):
        ...
        if self._enable_tar_obs:
            ...
            tar_root_pos_flat = torch.reshape(
                tar_root_pos,
                [tar_root_pos.shape[0] * tar_root_pos.shape[1], tar_root_pos.shape[-1]],
            )
            tar_root_rot_flat = torch.reshape(
                tar_root_rot,
                [tar_root_rot.shape[0] * tar_root_rot.shape[1], tar_root_rot.shape[-1]],
            )
            tar_joint_rot_flat = torch.reshape(
                tar_joint_rot,
                [
                    tar_joint_rot.shape[0] * tar_joint_rot.shape[1],
                    tar_joint_rot.shape[-2],
                    tar_joint_rot.shape[-1],
                ],
            )
            tar_body_pos_flat, _ = self._kin_char_model.forward_kinematics(
                tar_root_pos_flat, tar_root_rot_flat, tar_joint_rot_flat
            )
            tar_body_pos = torch.reshape(
                tar_body_pos_flat,
                [
                    tar_root_pos.shape[0],
                    tar_root_pos.shape[1],
                    tar_body_pos_flat.shape[-2],
                    tar_body_pos_flat.shape[-1],
                ],
            )
    ...
    def _fetch_tar_obs_data(self, motion_ids, motion_times):
        ...
        root_pos, root_rot, root_vel, root_ang_vel, joint_rot, dof_vel = (
            self._motion_lib.calc_motion_frame(motion_ids_tiled, motion_times)
        )

        root_pos = root_pos.reshape([n, num_steps, root_pos.shape[-1]])
        root_rot = root_rot.reshape([n, num_steps, root_rot.shape[-1]])
        joint_rot = joint_rot.reshape(
            [n, num_steps, joint_rot.shape[-2], joint_rot.shape[-1]]
        )
...
def convert_to_local(root_rot, root_vel, root_ang_vel, key_pos):
    ...
    if len(key_pos) > 0:
        ...
        flat_key_pos = key_pos.reshape(
            key_pos.shape[0] * key_pos.shape[1], key_pos.shape[2]
        )
        flat_local_key_pos = torch_util.quat_rotate(
            flat_heading_rot_expand, flat_key_pos
        )
        local_key_pos = flat_local_key_pos.reshape(
            key_pos.shape[0], key_pos.shape[1], key_pos.shape[2]
        )
    else:
...
def compute_tar_obs(
    ...
    joint_rot_flat = torch.reshape(
        joint_rot,
        [
            joint_rot.shape[0] * joint_rot.shape[1] * joint_rot.shape[2],
            joint_rot.shape[3],
        ],
    )
    ...
    joint_rot_obs = torch.reshape(
        ...
        [
            joint_rot.shape[0],
            joint_rot.shape[1],
            joint_rot.shape[2] * joint_rot_obs_flat.shape[-1],
        ],
    )

    obs = [root_pos_obs, root_rot_obs, joint_rot_obs]
    if len(key_pos) > 0:
        key_pos = torch.reshape(
            key_pos,
            [key_pos.shape[0], key_pos.shape[1], key_pos.shape[2] * key_pos.shape[3]],
        )
        obs.append(key_pos)
...
def compute_deepmimic_obs(
    ...
    char_obs = char_env.compute_char_obs(
        root_pos=root_pos,
        root_rot=root_rot,
        root_vel=root_vel,
        root_ang_vel=root_ang_vel,
    ...
    if enable_early_termination:
        ...
        if pose_termination:
            ...
            if not global_obs:
                ...
                tar_body_pos = char_env.convert_to_local_root_body_pos(
                    tar_root_rot, tar_body_pos
                )
            ...
            if track_root:
                ...
                root_pos_fail = (
                    root_pos_dist > pose_termination_dist * pose_termination_dist
                )
...
def compute_reward(
    ...
    if not track_root:
        root_rot, root_vel, root_ang_vel, key_pos = convert_to_local(
            root_rot, root_vel, root_ang_vel, key_pos
        )
        tar_root_rot, tar_root_vel, tar_root_ang_vel, tar_key_pos = convert_to_local(
            tar_root_rot, tar_root_vel, tar_root_ang_vel, tar_key_pos
        )
