...
class TaskSteeringEnv(amp_env.AMPEnv):
    ...
    def _build_markers(self, env_id):
        ...
        tar_marker_id = self._engine.create_obj(
            env_id=env_id,
        ...
        face_marker_id = self._engine.create_obj(
            env_id=env_id,
            obj_type=engine.ObjType.rigid,
            asset_file=asset_file,
            name="face_marker",
    ...
    def _build_sim_tensors(self, config):
        ...
        self._tar_dir = torch.zeros(
            [num_envs, 2], device=self._device, dtype=torch.float
        )
        self._face_dir = torch.zeros(
            [num_envs, 2], device=self._device, dtype=torch.float
        )
        self._tar_change_times = torch.zeros(
            [num_envs], device=self._device, dtype=torch.float
        )
        self._prev_root_pos = torch.zeros(
            [num_envs, 3], device=self._device, dtype=torch.float
        )
    ...
    def _update_marker(self, env_ids):
        ...
        tar_dist = (tar_speed - self._tar_speed_min) / (
    ...
    def _reset_task(self, env_ids):
        ...
        ) * rand_dt + self._tar_change_time_min
    ...
    def _update_reward(self):
        ...
        self._reward_buf[:] = compute_steering_reward(
            root_pos=char_root_pos,
