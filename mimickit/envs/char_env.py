...
class CharEnv(sim_env.SimEnv):
    ...
    def _parse_init_pose(self, init_pose, device):
        ...
        init_root_pos, init_root_rot, init_dof_pos = motion_lib.extract_pose_data(
            init_pose
        )
    ...
    def _build_sim_tensors(self, config):
        ...
        self._action_bound_low = torch.tensor(
            self._action_space.low, device=self._device
        )
        self._action_bound_high = torch.tensor(
            self._action_space.high, device=self._device
        )
    ...
    def _build_action_space(self):
        ...
        if control_mode == engine.ControlMode.none:
            ...
            control_mode == engine.ControlMode.pos
    ...
    def _update_done(self):
        self._done_buf[:] = compute_done(
            self._done_buf, self._time_buf, self._episode_length
        )
        return
    ...
    def _reset_char_rigid_body_state(self, env_ids):
        ...
        joint_rot = self._kin_char_model.dof_to_rot(dof_pos)
        body_pos, body_rot = self._kin_char_model.forward_kinematics(
    ...
    def _apply_action(self, actions):
        ...
        clip_action = torch.minimum(
            torch.maximum(actions, self._action_bound_low), self._action_bound_high
        )
    ...
    def _update_camera(self):
        if self._camera_mode is CameraMode.still:
            ...
            new_cam_pos = np.array(
                [char_pos[0] + cam_delta[0], char_pos[1] + cam_delta[1], cam_pos[2]]
            )
...
def convert_to_local_body_pos(root_rot, body_pos):
    ...
    flat_body_pos = body_pos.reshape(
        body_pos.shape[0] * body_pos.shape[1], body_pos.shape[2]
    )
...
def compute_char_obs(
    ...
    joint_rot_flat = torch.reshape(
        joint_rot, [joint_rot.shape[0] * joint_rot.shape[1], joint_rot.shape[2]]
    )
    ...
    if len(key_pos) > 0:
        ...
        key_pos_flat = torch.reshape(
            key_pos, [key_pos.shape[0], key_pos.shape[1] * key_pos.shape[2]]
        )
