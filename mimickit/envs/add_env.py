...
def compute_pos_obs(root_pos, root_rot, joint_rot, body_pos, global_obs):
    ...
    if not global_obs:
        ...
        heading_inv_rot_expand = heading_inv_rot_expand.repeat(
            1, 1, body_pos.shape[-2], 1
        )
        heading_inv_rot_expand_flat = heading_inv_rot_expand.reshape(
            (
        ...
        body_pos_flat = torch.reshape(
            body_pos,
            [
                body_pos.shape[0] * body_pos.shape[1],
                body_pos.shape[2],
                body_pos.shape[3],
            ],
        )
        body_pos_flat = torch_util.quat_rotate(
            heading_inv_rot_expand_flat, body_pos_flat
        )
    ...
    body_pos = torch.reshape(
        body_pos,
        [body_pos.shape[0], body_pos.shape[1], body_pos.shape[2] * body_pos.shape[3]],
    )
...
def compute_disc_obs(
    root_pos, root_rot, root_vel, root_ang_vel, joint_rot, dof_vel, body_pos, global_obs
):
