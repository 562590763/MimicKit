...
def plot_line(
    x_data, y_data, std_data=[], label="", line_style=None, color=None, draw_band=True
):
    ...
    if draw_band:
        ...
        if stds is not None:
            plt.fill_between(
                ...
                alpha=0.25,
                linewidth=0,
                facecolor=curr_line[0].get_color(),
                label="_nolegend_",
            )
    else:
        for i in range(len(x_data)):
            ...
            if i == 0:
                curr_line = plt.plot(
                    xs, ys, label=label, alpha=alpha, linestyle=line_style, color=color
                )
            else:
                curr_line = plt.plot(
                    xs,
